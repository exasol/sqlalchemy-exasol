...
class EXADialectTurbodbcTest(fixtures.TestBase):
    ...
    def _assert_connect_args(result, expected, expected_turbodbc):
        ...
        assert result[
            "turbodbc_options"
        ].read_buffer_size.megabytes == expected_turbodbc.get("read_buffer_size", 50)
        assert result[
            "turbodbc_options"
        ].parameter_sets_to_buffer == expected_turbodbc.get(
            "parameter_sets_to_buffer", 1000
        )
        assert result["turbodbc_options"].use_async_io == expected_turbodbc.get(
            "use_async_io", False
        )
    ...
    def assert_parsed(
        self, dsn, expected_connector, expected_args, expected_turbodbc_args
    ):
        url = sa_url.make_url(dsn)
    ...
    def test_create_connect_args(self):
        ...
        self.assert_parsed(
            "exa+turbodbc://scott:tiger@192.168.1.2..8:1234/my_schema",
            [None],
            expected,
            {},
        )
    ...
    def test_create_connect_args_dsn_without_user(self):
        self.assert_parsed(
            "exa+turbodbc://exa_test", ["exa_test"], DEFAULT_CONNECTION_PARAMS, {}
        )
    ...
    def test_create_connect_args_dsn(self):
        ...
        expected.update(DEFAULT_CONNECTION_PARAMS)
        self.assert_parsed(
            "exa+pyodbc://scott:tiger@exa_test", ["exa_test"], expected, {}
        )
    ...
    def test_create_connect_args_trusted(self):
        ...
        expected.update(DEFAULT_CONNECTION_PARAMS)
        self.assert_parsed(
            "exa+pyodbc://192.168.1.2..8:1234/my_schema", [None], expected, {}
        )
    ...
    def test_create_connect_args_with_custom_parameter(self):
        ...
        assert (
            self._get_connection_arguments(base_url + "?CUSTOM=something")["custom"]
            == "something"
        )
    ...
    def test_create_connect_args_with_parameter_set_to_none(self):
        base_url = "exa+pyodbc://scott:tiger@192.168.1.2..8:1234/my_schema"
        assert (
            self._get_connection_arguments(base_url + "?CUSTOM=None")["custom"] is None
        )
    ...
    def test_create_connect_args_overrides_default(self):
        ...
        after = self._get_connection_arguments(
            base_url + "?inttypesinresultsifpossible=custom"
