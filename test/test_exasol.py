...
class MergeTest(fixtures.TablesTest):
    ...
    def define_tables(cls, metadata):
        ...
        Table("s", metadata, Column("id", Integer), Column("age", Integer))

    def test_merge_update(self):
        t = self.tables.t
        s = self.tables.s

        config.db.execute(t.insert(), [dict(id=1, age=None, name="Ulf")])
        config.db.execute(s.insert(), [dict(id=1, age=10)])
    ...
    def test_merge_update_where(self):
        ...
        m = merge(t, s, t.c.id == s.c.id).update(
            values={t.c.age: s.c.age}, where=t.c.id == s.c.id
        )
        mr = config.db.execute(m)
    ...
    def test_merge_delete_where(self):
        ...
        m = merge(t, s, t.c.id == s.c.id).insert(where=s.c.age > 5)
        config.db.execute(m)
        r = config.db.execute(t.select()).fetchall()
        assert len(r) == 2
    ...
    def test_merge_update_insert(self):
        ...
        m = merge(t, s, t.c.id == s.c.id).update(values={t.c.age: s.c.age}).insert()
        config.db.execute(m)
        r = config.db.execute(t.select()).fetchall()
        assert len(r) == 2
    ...
    def test_merge_update_delete(self):
        ...
        m = merge(t, s, t.c.id == s.c.id).update(values={t.c.age: s.c.age}).delete()
        config.db.execute(m)
        r = config.db.execute(t.select()).fetchall()
        assert len(r) == 0
    ...
    def test_merge_update_delete_where(self):
        ...
        m = (
            merge(t, s, t.c.id == s.c.id)
            .update(values={t.c.age: s.c.age})
